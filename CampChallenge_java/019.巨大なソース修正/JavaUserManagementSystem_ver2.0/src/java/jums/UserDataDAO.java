...
public class UserDataDAO {
        ...
        try{
            ...
            if (ud.getType()!=0) {
                if(!flag){
                    ...
                    sql += " AND type like ?";
                    sType = (sBirthday == 2) ? 3 : 2; 
                }
            }
            st =  con.prepareStatement(sql);
            if(sName != 0){
                st.setString(sName, "%"+ud.getName()+"%");
            }
            if(sBirthday != 0){
                st.setString(sBirthday, "%"+ new SimpleDateFormat("yyyy").format(ud.getBirthday())+"%");
            }
            if(sType != 0){
                st.setInt(sType, ud.getType());
            }
            
            ResultSet rs = st.executeQuery();
            ArrayList<UserDataDTO> idDTO = new ArrayList<UserDataDTO>();
            while(rs.next()){
     ...
     * @throws SQLException 呼び出し元にcatchさせるためにスロー 
     */
    public void Correction() throws SQLException{
        Connection con = null;
        PreparedStatement st = null;
        try{
            con = DBManager.getConnection();
            ...
            sql = "UPDATE user_t SET userID = (@i := @i + 1)";
            st =  con.prepareStatement(sql);
            st.executeUpdate();
            ...
            ResultSet rs = st.executeQuery();
            rs.next();
            int nextIncrement = rs.getInt(1);
            
            sql = "ALTER TABLE user_t AUTO_INCREMENT = ?";
            st =  con.prepareStatement(sql);
            st.setInt(1,nextIncrement);
            st.executeUpdate();
